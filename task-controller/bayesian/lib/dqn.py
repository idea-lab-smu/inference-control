"""
	Originally implemented by Giacomo Spigler

	modified by jeehanglee@gmail.com
...
reward_clipping: 
	- any negative number disables rewards clipping. 
	- Positive numbers mean that the rewards will be clipped to be in [-reward_clipping, reward_clipping]
"""

class DQN(object):

	def __init__(self, state_size,								# size of the state in the environment
					   action_size,								# size of actions (it depends on the number of nodes)
		...
		self.qnet = QNetwork_BML(self.state_size, self.action_size, "qnet")
		self.target_qnet = QNetwork_BML(self.state_size, self.action_size, "target_qnet")

		# set optimiser for a backpropagation
	...
	def create_graph(self):
		...
		with tf.name_scope("pick_action"):
			...
			# save max q-values to track learning
		...
		with tf.name_scope("estimating_future_rewards"):
			...
			# In practice, we use the actual QNet (non target) to select the action for the next state, 
			#	but then use its Q value estimated using the target network
			...
			if self.DoubleDQN:
				...
				self.next_max_q_values = tf.stop_gradient( tf.reduce_sum( tf.multiply( self.next_q_values_targetqnet, \
							self.next_selected_actions_onehot ) , reduction_indices=[1,] ) * self.next_state_mask )
	...
	def action(self, state, training = False):
		...
		"""
		If `training', compute the epsilon-greedy parameter epsilon according to 
			the defined exploration_period, initial_epsilon and final_epsilon.
