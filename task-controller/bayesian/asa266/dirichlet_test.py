...
from asa266_dirichlet import Dirichlet
...
if __name__ == "__main__":
	print LF
	print 'Dirichlet Test Program'
	print ID, 'For samples of a Dirichlet PDF'
	print ID, 'DIRICHLET_ESTIMATE estimates the parameters (e.g. alpha)'
	print ID, 'DIRICHLET_MEAN finds the means'
	print ID, 'DIRICHLET_VARIANCE finds the variances', LF
...
print ID, 'Sampled Data :', LF
print_matrix(sample, '\t%.6f')

#
# compute the observed mean and variance
#
print ID, 'Observed means and variances are:'
for i in range(len(sample)):
	obs_mean = np.mean(sample[i])
	obs_var = np.var(sample[i])
	print '%6d%14.8f%14.8f' % (i, obs_mean, obs_var)
print LF
...
#
print ID, 'Index, Estimate, Lower Limit, Upper Limit:'
row = len(sample)
for i in range(row):
	vari = v[i + i * row]
	aminus = (alpha[i] - 1.96 * math.sqrt(vari)) 
	aplus = (alpha[i] + 1.96 * math.sqrt(vari)) 
	print '%6d%14.8f%14.8f%14.8f' % (i, alpha[i], aminus, aplus)
print LF

#
# Dirichlet mean and variance
#
print ID, 'Expected means and variances are:'
for i in range(row):
	mean = alpha[i] / alpha_sum
	alpha_mean.insert(i, mean)

	variance = alpha[i] * (alpha_sum - alpha[i])\
				/ (alpha_sum * alpha_sum * (alpha_sum + 1.0))
	alpha_var.insert(i, variance)

	print '%6d%14.8f%14.8f' % (i, mean, variance)
print LF

print ID, 'Alpha sum is %.8f' % alpha_sum
print LF
...
for i in range(row):
	...
	print '%6d%14.8f%14.8f%14.8f' % (i, alpha[i] / alpha_sum, aminus, aplus)
print LF

print ID, 'Log likelihood function = %.8f' % rlogl
print LF
