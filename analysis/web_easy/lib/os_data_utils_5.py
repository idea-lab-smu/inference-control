...
import total_code.total_easy.easy.lib.score_efficiency as eff
...
def paired_t_test(data, useRandom):
	...
	if useRandom == True:
		...
		pval_buf.append(p_val)
		#min random
		t_stat, p_val = stats.ttest_rel(data[2], data[3], nan_policy='omit')
...
#medium mode
def significance(ax, data, pval, rects, rect_idx, ylim = 12.0, step = 1.0, useRandom = True):
	#max no min random
	if useRandom == True:
		order = [[0, 1], [1, 2], [0, 2], [1, 3], [0, 3], [2, 3]]
	else:
...
def draw_trial_zscore(num_nodes, trial_buf, efficiency, plt_metric):
	...
	bayes_z = np.mean(trial_buf[0])
	max_os_z = np.mean(trial_buf[1])
	min_os_z = np.mean(trial_buf[2])
	
	if useRandom == True:
		rand_os_z = np.mean(trial_buf[3])
		
	z_buf = stats.zscore([max_os_z, bayes_z, min_os_z, rand_os_z])
	
	rects = plt.bar(np.arange(4), z_buf, 0.8, color = 'tab:gray')
	
	xticks = ['Oneshot+', 'Bayesian+', 'Oneshot-', 'Random']
...
def draw_trial_sem(num_nodes, trial_buf, efficiency, plt_metric,  useRandom, bsem = True):
	...
	if useRandom == True:
		rects3 = ax.bar(ind + (width + gap) * 2, min_os_mean, width, yerr=min_os_err,
						color=bar_color)  # , label = 'min oneshot')
	...
	if pVal < 0.05:
		...
		data['group'] = [['max'] * len(trial_buf[1]), ['no'] * len(trial_buf[0]), ['min'] * len(trial_buf[2]),
						 ['random'] * len(trial_buf[3])]
		comp = MultiComparison(data['value'], data['group'])
		# print("comp: ", comp)
		result = comp.allpairtest(stats.ttest_rel, method='bonf')
...
def distinct_ic_os_buf(conf_map, efficiency, distance):
	...
	for cm in conf_map:
		...
		#visit_cnt_no_display = eff.visits_on_node_5(seq_id, True)
