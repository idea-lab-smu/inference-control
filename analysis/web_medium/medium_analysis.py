...
import copy
filename = './web_medium/medium_pickle/medium-task_user_%d.pickle'
...
def result_cmaps_for_zscore(cls, outlier, efficiency, distance, rd_list=[], rd_indices=[]):
    ...
    for idx in range(N - B):
        ...
        for j in range(start_trial, end_trial):
            ...
            if seq1 == 4:
                rd_idx = rd_list.index(seq_detail1.tolist())
...
def result_cmaps(cls, outlier, efficiency, distance, rd_list=[], rd_indices=[]):
    seq = []
    seq_detail = []
    rating = []
    seqhist = []
...
def result(cls, outlier, efficiency, distance, rd_list=[], rd_indices=[]):
    ...
    conf_map = result_cmaps(cls, outlier, efficiency, distance, rd_list = rd_list, rd_indices = rd_indices)
...
def get_webmedium_result(cls, outlier, efficiency, distance, rd_list=[], rd_indices=[]):
...
def set_plt_metric_random(efficiency, type):
    plt_metric = {'ylow': 4.0, 'ylim': 8.0, 'step': 1.0, 'visit_cnt': 20, 'std': 19.0, 'figx': 4, 'figy': 3}

    if efficiency == False:
        plt_metric['ylim'] = 5.8 if type == 'confidence' else 5.0
        plt_metric['ylow'] = 4.0 if type == 'confidence' else 3.0
        plt_metric['visit_cnt'] = 20

    else:
        plt_metric['ylim'] = 2.5 if type == 'confidence' else 2.2
        plt_metric['ylow'] = 1.0 if type == 'confidence' else 0.5
