...
def z_plot_opt_vs_counteropt(cls, num_nodes, outlier, efficiency, distance, rd_list = [], rd_indices = []):
	...
	web_rating_list = we.result_cmaps_for_zscore(cls, outlier, efficiency, distance, rd_list = rd_list)
...
def main():
	...
	#z_plot_opt_vs_counteropt([], 5, [], False, distance, rd_list = rd.seq_random_5)
	z_plot_opt_vs_counteropt([], 5, [], False, distance, rd_list = [])
	
	#cls = [1, 2, 3, 4, 6, 11, 12, 16, 19, 20, 21, 24] # oneshot index - upper 50 (the larger, the better)
	#z_plot_opt_vs_counteropt(cls, 5, [], False, distance, rd_list = rd.seq_random_5)
